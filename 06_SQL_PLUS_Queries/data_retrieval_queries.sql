...
SELECT customer_key, customer_id, first_name, last_name, gender
FROM customer_dim
WHERE current_flag = 'Y';

-- Retrieve active policies
SELECT policy_key, policy_id, policy_type, policy_status
FROM policy_dim
WHERE current_flag = 'Y';
...
-- Claims by year
SELECT d.year, COUNT(*) AS total_claims
FROM claim_fact cf
...
SELECT pd.policy_type, SUM(pf.premium_amount) AS total_premium
FROM policy_fact pf
...
SELECT customer_id, address, effective_start_dt, effective_end_dt, current_flag
FROM customer_dim
